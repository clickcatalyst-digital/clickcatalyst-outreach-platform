...
The 2026-06-20 "cloud migration" (future work #1 below) is **DONE**: Turso is the shared DB, the Next.js dashboard runs on Render (US-only, Basic Auth, server-side Turso route handlers), workers write Turso, FastAPI stays local. The US engine is **autonomous** — `com.clickcatalyst.us` is loaded and ticking every 20 min; verified end-to-end (dashboard config write → orchestrator reads it next cycle → sends → writes Turso → dashboard reflects). Now in **PROD** (live sends to real prospects, warmup-capped ~5/day); toggle Mode on the US Outreach page. **Open-tracking live.** **Reply tracking paused** — Gmail refresh token revoked; re-auth via `python reply_tracker.py` (the code now self-heals instead of crashing). Both launchd daemons (heartbeat 60s, engine 20 min) auto-load on login/reboot and are firing (run counts climbing). `tick.sh` waits up to 60s for internet before running (robust when a tick fires right after wake — previously the tracking sync failed on not-ready DNS and burned its 300s retry budget). launchd timers pause while the Mac sleeps and catch up on wake; for 24/7 keep it on AC + `sudo pmset -c sleep 0`. **Day-to-day runbook: [`OPERATIONS.md`](OPERATIONS.md)** (status checks, common actions, troubleshooting).

**Diagnostic note — dashboard staleness (proven root cause):** there is a SINGLE Turso DB (`cat-mail-db`); workers and dashboard use the same `TURSO_URL`/token (verified — a direct `@libsql` read with the dashboard's creds returns fresh). Two stale-*display* causes, neither a code/DB bug: (1) the **engine daemon isn't running** (load `com.clickcatalyst.us`) — the real "Last ran 20h ago"; (2) **Next.js's persistent `.next/cache`** serving an old route render — `next build` reuses that cache across builds, so a stale snapshot survives redeploys until cleared (proven: `rm -rf .next` + rebuild → fresh). Fixes applied: the `json()` helper in `dashboard/app/lib/turso.server.js` sets `Cache-Control: no-store` (stops browser/proxy caching) and routes are `force-dynamic`; on Render you must do **Manual Deploy → Clear build cache & deploy** once to drop the stale `.next/cache`.
